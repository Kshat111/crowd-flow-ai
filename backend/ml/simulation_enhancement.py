...
def create_enhanced_top_view(frame, density_map, previous_density_map, person_detector, homography, area_manager, 
    ...
    if density_map is not None:
        ...
        # CHANGED: Use a much higher contrast custom color mapping for better visibility against red environment
        # Create a custom colormap that's primarily cyan, bright green and yellow for maximum contrast
        ...
        # ENHANCEMENT: Generate points based on density map
...
def create_flow_visualization(frame, density_map_current, density_map_previous, person_detector, 
    ...
    update_interval = 5      # Minimum frames between direction updates
    smoothing_factor = 0.2    # How quickly to blend in new direction (0.1=slow, 0.9=fast)
            ...
            if len(track) > 4:  # Need at least 5 points for stable movement
                ...
                if len(top_view_track) >= 2:
                    ...
                    if vector_length > 10:
                        ...
                        color = person_detector._get_color_by_id(track_id)
                        movement_vectors.append((end_point, norm_vector, color))
    
    # Draw individual movement vectors
    for pos, vector, color in movement_vectors:
    ...
    if len(movement_vectors) >= 3:
        avg_x = sum(v[0] for _, v, _ in movement_vectors) / len(movement_vectors)
        avg_y = sum(v[1] for _, v, _ in movement_vectors) / len(movement_vectors)
        
        # Calculate center position
        center_x = sum(p[0] for p, _, _ in movement_vectors) / len(movement_vectors)
        center_y = sum(p[1] for p, _, _ in movement_vectors) / len(movement_vectors)
...
def enhanced_process_cctv_to_top_view(video_path, output_path=None, calibration_image=None, 
    ...
    cv2.namedWindow('Enhanced Top View', cv2.WINDOW_NORMAL)
    ...
    while cap.isOpened():
        ...
        # Display top view
        cv2.imshow('Enhanced Top View', top_view)
